/**
 * Update Top 10 MCU Villains article with expanded Secret Wars content — May 6, 2026
 * Run from project root: node update-top10-villains.mjs
 */
...
const SLUG = "top-10-most-popular-mcu-villains-all-time";

const FEATURED_IMAGE = "https://d2xsxph8kpxj0f.cloudfront.net/310419663027009739/SGHqXeh8PZJcCDnFiAMuFi/nlf-top10-mcu-villains-DHSSC9rBiiS9kojewPds29.png";

const newTitle = "THE TOP 10 MOST POPULAR MCU VILLAINS OF ALL TIME — FROM SECRET WARS TO DOOMSDAY";

const newExcerpt = "From the 1984 Secret Wars to Avengers: Doomsday, these are the 10 MCU villains who terrified audiences, broke box offices, and are now dominating the trading card market. Doctor Doom takes the crown.";

const newMetaDescription = "The definitive ranking of the 10 greatest MCU villains of all time. Doctor Doom claims #1 ahead of Thanos, with card market analysis for every villain on the list. Includes 1984 and 2015 Secret Wars comic history.";

const newTags = JSON.stringify(["Top 10", "Villains", "Doctor Doom", "Thanos", "Secret Wars", "Doomsday", "MCU", "Card Market"]);

const newRelatedCharacters = JSON.stringify(["Doctor Doom", "Thanos", "Loki", "Killmonger", "Hela", "Vulture", "Green Goblin", "Ultron", "Wenwu", "Kingpin"]);

const newCardMarketImpact = "Doctor Doom and Thanos cards are surging ahead of Avengers: Doomsday. Doom cards have seen 300%+ gains since RDJ's casting was announced, while Killmonger and Hela remain undervalued relative to their cultural impact.";

const newSources = JSON.stringify([
...
const newContent = `The MCU has given us some of the greatest villains in cinema history. From genocidal titans to gods of mischief, from crime lords to multiversal conquerors — Marvel's rogues gallery is stacked. But who stands at the top?
...
async function main() {
  ...
  const [rows] = await conn.execute(
    "SELECT id, title, slug, publishedAt FROM articles WHERE slug = ?",
    [SLUG]
  );
  console.log("\n--- Updated Article ---");
  rows.forEach((r) => console.log(`  ID: ${r.id} | Title: ${r.title} | Slug: ${r.slug}`));
