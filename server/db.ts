...
/**
 * Card of the Day - deterministic daily card based on the current date.
 * Uses a hash of the date string to pick a consistent card for the whole day.
 */
export async function getCardOfTheDay(): Promise<{
  ...
  const dayIndex = Math.abs(hash) % totalCards;

  // Pick the card at that index, preferring cards with images
  const result = await db.select({
    id: marvelCards.id,
    cardNumber: marvelCards.cardNumber,
    characterName: marvelCards.characterName,
    ...
    .innerJoin(marvelSets, eq(marvelCards.setId, marvelSets.id))
    .where(sql`${marvelCards.imageUrl} IS NOT NULL AND ${marvelCards.imageUrl} != ''`)
    .orderBy(asc(marvelCards.id))
    .limit(1)
    .offset(dayIndex % totalCards);

  // Fallback: if offset is beyond image-having cards, get any card with image
  if (!result[0]) {
