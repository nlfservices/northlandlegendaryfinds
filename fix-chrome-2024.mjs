...
// The correct set ID for 2024 Topps Chrome Marvel is 90001
const CHROME_MARVEL_ID = 90001;

// First check what's currently in 90001
const [existing] = await conn.execute('SELECT COUNT(*) as cnt FROM marvel_cards WHERE setId = ?', [CHROME_MARVEL_ID]);
console.log(`Current cards in set 90001: ${existing[0].cnt}`);

// Clear any existing cards
await conn.execute('DELETE FROM marvel_cards WHERE setId = ?', [CHROME_MARVEL_ID]);
console.log('Cleared existing cards for set 90001');
...
for (const [num, name] of chromeBase) {
  chromeCards.push({ cardNumber: String(num), name, cardType: 'Base', rarity: 'Base' });
}

// Artist Autographs
for (const [code, name] of chromeArtistAutos) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Comic Artist Autograph', rarity: 'Autograph /99' });
}

// Frank Miller Sketch
chromeCards.push({ cardNumber: 'FM-SK', name: 'Frank Miller Daredevil 60th Anniversary Sketch', cardType: 'Sketch Card', rarity: 'Sketch 1:14,457' });

// Character Autographs
for (const [code, name] of chromeCharAutos) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Character Autograph (Facsimile)', rarity: 'Autograph /99' });
}

// Dual Autographs
for (const [code, name] of chromeDualAutos) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Dual Autograph', rarity: 'Autograph /25' });
}

// Quad Autograph
for (const [code, name] of chromeQuadAuto) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Quad Autograph', rarity: 'Autograph /10' });
}

// Daredevil 60th Anniversary Insert
for (const [code, name] of chromeDaredevil60th) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Daredevil 60th Anniversary', rarity: 'Insert 1:12' });
}

// Marvel 90s Insert
for (const [code, name] of chromeMarvel90s) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Marvel 90s', rarity: 'Insert 1:12' });
}

// Giant-Sized 50th Anniversary Insert
for (const [code, name] of chromeGiantSized) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Giant-Sized 50th Anniversary', rarity: 'Insert 1:12' });
}

// Giant-Sized 50th Anniversary Box Toppers
for (const [code, name] of chromeGiantSizedBoxToppers) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Giant-Sized 50th Box Topper', rarity: '1 per Hobby Box' });
}

// Marvel Icons Insert
for (const [code, name] of chromeIcons) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Marvel Icons', rarity: 'Insert 1:12' });
}

// Spider-Man Gold Insert
for (const [code, name] of chromeSpiderGold) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Spider-Man Gold', rarity: 'Insert 1:72' });
}

// Avengers Infinity (Scarce)
for (const [code, name] of chromeAvengersInfinity) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Avengers Infinity', rarity: 'Black Wave /10 - 1:9,036' });
}

// Indestructible (Scarce)
for (const [code, name] of chromeIndestructible) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Indestructible', rarity: 'Black Lazer /10 - 1:4,566' });
}

// Mask On, Mask Off (Scarce)
for (const [code, name] of chromeMaskOnOff) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Mask On, Mask Off', rarity: 'Black Wave /10 - 1:5,043' });
}

// Wolverine 50th Anniversary (Scarce)
for (const [code, name] of chromeWolverine50th) {
  chromeCards.push({ cardNumber: code, name, cardType: 'Wolverine 50th Anniversary', rarity: 'Black Wave /10 - 1:9,036' });
}

console.log(`Total cards to insert: ${chromeCards.length}`);

// Insert in batches of 50
const BATCH_SIZE = 50;
let inserted = 0;
for (let i = 0; i < chromeCards.length; i += BATCH_SIZE) {
  const batch = chromeCards.slice(i, i + BATCH_SIZE);
  const values = batch.map(c => [CHROME_MARVEL_ID, c.cardNumber, c.name, c.cardType, c.rarity, null]);
  ...
  console.log(`  Inserted ${inserted}/${chromeCards.length}`);
}

// Update set card count
await conn.execute('UPDATE marvel_sets SET totalCards = ? WHERE id = ?', [chromeCards.length, CHROME_MARVEL_ID]);

console.log(`\n✅ Done! Inserted ${chromeCards.length} cards into 2024 Topps Chrome Marvel (set ID ${CHROME_MARVEL_ID})`);
await conn.end();
