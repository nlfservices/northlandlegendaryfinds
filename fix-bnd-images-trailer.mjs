/**
 * Fix Spider-Man: Brand New Day article:
 * 1. Move YouTube trailer embed right after opening paragraph (near top)
 * 2. Replace duplicate image (cards section was using same hero image) with new unique image
 */
...
const IMAGES = {
  ...
  cards: "https://d2xsxph8kpxj0f.cloudfront.net/310419663027009739/SGHqXeh8PZJcCDnFiAMuFi/spiderman-bnd-cards-market-v2-8CVFrkd7XLVShWztMMZMQe.webp",
};
...
<img src="${IMAGES.villain}" alt="Armored villain in dark alley — Marvel cinematic concept art" style="width:100%;border-radius:8px;margin:2rem 0;" />
...
<img src="${IMAGES.cards}" alt="Marvel trading cards collection on velvet surface with holographic chrome refractors" style="width:100%;border-radius:8px;margin:2rem 0;" />
...
async function main() {
  ...
  const [result] = await conn.execute(
    ...
    [correctedContent, "spiderman-brand-new-day-trailer-record-breaking-card-market"]
  );

  console.log(`✅ Article updated: ${result.affectedRows} row(s) affected`);
  console.log("   - Trailer moved to right after opening paragraph");
  console.log("   - Duplicate image replaced with unique cards/collector image");
