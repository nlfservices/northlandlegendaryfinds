# 🦸‍♂️ STAN LEE - Northland Legendary Finds Task List

**Trigger phrase:** "Simon says Stan Lee"

---

## 📋 Incomplete Tasks

### 🔔 GoHighLevel Email Workflow
**Status:** Started, needs completion  
**What's done:**
- ✅ Workflow created
- ✅ Trigger: Form Submitted
- ✅ Email action added
- ✅ From: admin@nlfservices.com
- ✅ Subject: "Welcome to Northland Legendary Finds! 🎉"

**What's needed:**
- [ ] Add email body text to welcome email
- [ ] Click "Save Action"
- [ ] Add second email action (notification to you when someone signs up)
- [ ] Publish/activate the workflow

**Email body to add:**
```
Hey there!

Thanks for joining our early access list for Marvel & Star Wars trading card repacks!

You'll be the FIRST to know when we launch on Friday, March 13th, 2026 at 7:00 PM Central Time.

Plus, you'll get an exclusive discount code on launch day!

Get ready for something legendary! 🚀

See you soon,
Northland Legendary Finds Team
```

---

### 📊 Facebook Pixel Installation
**Status:** Not started  
**Purpose:** Track visitors for retargeting ads

**What's needed:**
- [ ] Get Facebook Pixel ID from Events Manager
- [ ] Install pixel code on countdown page
- [ ] Add custom events (form submission, button clicks)
- [ ] Create custom audiences in Facebook Ads Manager

**Retargeting strategy:**
- **Phase 1 (Pre-launch):** Retarget countdown page visitors with teaser ads
- **Phase 2 (Launch day):** Retarget non-buyers with "We're live!" ads
- **Phase 3 (Post-launch):** Cross-sell and last chance offers

---

### 🛒 E-Commerce Store Build
**Status:** Not started  
**Purpose:** Full shopping cart and checkout for card repacks

**What's needed:**
- [ ] Upgrade to full-stack (database + Stripe payments)
- [ ] Design product catalog structure (Marvel, Star Wars, Mixed)
- [ ] Create product pages with images and descriptions
- [ ] Build shopping cart functionality
- [ ] Implement Stripe checkout
- [ ] Set up order confirmation emails
- [ ] Create customer account system
- [ ] Build admin dashboard for inventory management

---

### 🎯 Marketing & Launch Prep
**Status:** Planning phase

**What's needed:**
- [ ] Create retargeting ad campaigns in Facebook
- [ ] Design ad creatives (images/videos)
- [ ] Set up email sequences in GoHighLevel
- [ ] Create launch day discount codes
- [ ] Plan social media posts
- [ ] Prepare product photography

---

## 📝 Notes

- **Countdown page is LIVE:** https://www.northlandlegendaryfinds.com
- **Owner bypass URL:** https://www.northlandlegendaryfinds.com?preview=admin
- **Launch date:** Friday, March 13th, 2026 at 7:00 PM Central Time
- **SSL:** ✅ Secured with HTTPS
- **Form:** ✅ Connected to GoHighLevel

---

**Last updated:** February 19, 2026
